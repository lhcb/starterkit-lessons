...
{% discussion "Example: Locating TES interactions" %}
```term
local:dir $ egrep 'get|put' Pr/PrAlgorithms/src/PrMatchNN.cpp
...
{% discussion "Example: Member declaration" %}
#### Member declaration:
...
{% discussion "Example: Property conversion" %}
#### Old header
...
In most cases, this change is transparent and you can use the property the same way. In cases where it does not work, such as when using `->` for a pointer property, you can use `.value()->` instead. Also, printing a property to an iostream is customized, giving name and value. If the old print is needed, use `.value()` to output just the value.
...
Moving to the new framework *should* take care of thread safety by using `const`, but there are a few ways around it that, unfortunately, are common in our code base. You can remove `const` with `const_cast`, and a member variable can be changed in a `const` method if it is declared `mutable`. As a general rule, you should only see `mutable` in C++11 when the variable itself is thread-safe (for example, a mutex). It should *not* be used to simply make adapting bad code easier. To remove
