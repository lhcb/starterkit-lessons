...
```bash
lb-run DaVinci/v45r6 gaudirun.py
```

This will run the `gaudirun.py` command using version v45r6 of DaVinci. (`lb-run` sets the specified environment for `gaudirun.py` to run in.)
`gaudirun.py` is a script that sets up the EventLoop.
You should get the following output:

```
====================================================================================================================================
                                                   Welcome to DaVinci version v45r6
                                          running on lxplus707.cern.ch on Mon Nov 30 14:17:08 2020
====================================================================================================================================
...
```bash
lb-run DaVinci/v45r6 gaudirun.py options.py
```
...
```bash
lb-run DaVinci/v45r6 gaudirun.py options1.py options2.py
```
...
These lessons use [DaVinci v45r6][v45r6], which was the latest Run 1/2 version at the
time the text was last revised.

[releases]: http://lhcbdoc.web.cern.ch/lhcbdoc/davinci/releases/
[v45r6]: http://lhcbdoc.web.cern.ch/lhcbdoc/davinci/releases/v45r6/

**Note:** Older versions of DaVinci may not be available on the default platform ```x86_64-centos7-gcc9-opt```.
...
```bash
lb-run DaVinci/v45r6 bash --norc
```
...
For most purposes, `SetupProject DaVinci v45r6` is equivalent to
```bash
lb-run DaVinci/v45r6 $SHELL
```
but you should really avoid doing things this way as this method is no longer supported for the latest project releases. (The environment for DaVinci v45r6, for example, cannot be started this way.)
